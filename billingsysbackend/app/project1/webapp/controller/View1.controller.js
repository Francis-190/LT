sap.ui.define([
    ...
    let sBase = "";
    if (oODataModel) {
        if (typeof oODataModel.getServiceUrl === "function") sBase = oODataModel.getServiceUrl();
        else if (oODataModel.sServiceUrl) sBase = oODataModel.sServiceUrl;
    }
    ...
    try {
        ...
        sap.m.MessageToast.show(`Updated model ${sModelCode}`);

        // Recalculate totals using oldQty and newQty
        this._recalculateTotalsForRow(oRowData, oldQty);

        // Update original quantity after recalculation
        ...
        sap.m.MessageToast.show("Failed to update. " + (err.message || ""));
        oButton.setEnabled(true);
    }
},

// Updated totals function to accept old quantity
